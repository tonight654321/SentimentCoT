...
def extract_polarities(cot_text):
    # 提取COT:中的情感极性
    aspect_polarity_pattern = re.compile(r'(anxiety|loneliness|fear|happiness|shame|jealousy)')
...
# 读取XML文件并提取句子和标签
def read_xml(prompt_name, file_path, n=10):
...
def reverse_aspects(prompt_text, prompt_name):
    def reverse_label(label):
        emotions = ["anxiety", "loneliness", "fear", "happiness", "shame", "jealousy"]
        if label in emotions:
            remaining_emotions = [emotion for emotion in emotions if emotion != label]
            return random.choice(remaining_emotions)
        return label
    ...
    def reverse_cot_v1(cot_text):
        ...
        def replace_func(match):
            label = match.group(0)
            if label not in label_replacements:
                label_replacements[label] = reverse_label(label)
            return label_replacements[label]
        new_text = re.sub(r'\b(anxiety|loneliness|fear|happiness|shame|jealousy)\b', replace_func, cot_text)
        return new_text

    def reverse_cot_v2(cot_text):
        return re.sub(r'\((\s*(anxiety|loneliness|fear|happiness|shame|jealousy)\s*(?:,\s*(anxiety|loneliness|fear|happiness|shame|jealousy)\s*)*)\)',
...
if __name__ == "__main__":
    ...
    parser.add_argument('--r_file', type=str, default='../data/mes_implicit.xml', help='Path to the input XML file')
    ...
    # 读取XML文件中的句子和标签
    text_list, aspect_polarity_truth, final_polarity_truth = read_xml(prompt_name, r_file, n=tested_num_each)
