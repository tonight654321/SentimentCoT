...
def read_xml(prompt_name, file_path, n=10, shuffle_input=False):
    ...
    one_aspect_sentence, one_final_polarity_truth = [], []
    two_aspect_sentence, two_final_polarity_truth = [], []
    three_aspect_sentence, three_final_polarity_truth = [], []
    four_aspect_sentence, four_final_polarity_truth = [], []
    ...
    for idx, sentence in enumerate(root.findall('sentence')):
        implicit_sentiment = list(
            set([aspect.get('implicit_sentiment') for aspect in sentence.findall('aspectTerms/aspectTerm')]))
        # 只处理纯隐式或纯显式的句子
        if len(implicit_sentiment) == 1 and implicit_sentiment[0] == implicit:
            ...
            if len(polarities) == 1 and len(one_aspect_sentence) < n:
                ...
                # 只有在 shuffle_input 为 True 时才打乱句子
                if shuffle_input:
...
def extract_indices_and_tokens(input_and_out_sequence, decoded_tokens, num_shots, model_name):
    ...
    for index, (token_id, decoded_token) in enumerate(zip(input_and_out_sequence, decoded_tokens)):
        if index == start_index:
            ...
            if (model_type == "gemma" and (decoded_token == "\n" or token_id == 1)) or (model_type == "llama" and decoded_token == "Sent"):
                break
    ...
    for index, (token_id, decoded_token) in enumerate(zip(input_and_out_sequence, decoded_tokens)):
        if index == input_indices[-1] + 3:
            ...
            if (model_type == "gemma" and (decoded_token == "\n" or token_id == 1)) or (model_type == "llama" and token_id == 198):
                break
...
if __name__ == "__main__":
    ...
    parser.add_argument('--r_file', type=str, default='../data/Restaurants_Test_Train_Gold_Implicit_Labeled.xml',
    ...
    if args.shuffle_input:
        output_file_json = os.path.join(result_dir,
                                    f"results_shuffle_{prompt_name}_{args.num_of_shots}-shot_{model_name}_{timestamp}.jsonl")
    else:
        output_file_json = os.path.join(result_dir,
                                        f"results_{prompt_name}_{args.num_of_shots}-shot_{model_name}_{timestamp}.jsonl")
    output_file_figure = figure_dir

    # 读取XML文件中的句子和标签
    text_list, final_polarity_truth = read_xml(prompt_name, r_file, n=tested_num_each,shuffle_input=args.shuffle_input)
